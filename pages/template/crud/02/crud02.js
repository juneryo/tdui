	...
	var vAgtManage = avalon.define({
		...
		$panel_opt: {
			buttons: [{display: '添加', icon: 'glyphicon glyphicon-plus', fun: function(ev, vm) {
				...
				if(avalon.vmodels['crud02_datagrid'].getSelectedIdx().length > 1) {
					TD.hint('每次只允许修改一条记录');
				}else if(avalon.vmodels['crud02_datagrid'].getSelectedIdx().length == 0) {
					TD.hint('请选择需修改的记录');
				}else {
					TD.alert('双击记录即可进行修改');
				}
			}}, {display: '删除', icon: 'glyphicon glyphicon-remove', fun: function(ev, vm) {
				if(avalon.vmodels['crud02_datagrid'].getSelectedIdx().length > 0) {
					TD.confirm('<strong style="color:red;">确认要删除选中数据吗？</strong>', function() {
						avalon.vmodels['crud02_datagrid'].removeSelectedRow();
					});
				}else {
					TD.hint('请选择需删除的数据');
				}
		...
		$qry_form_opt: {
			onsubmited: function(ev, vm) {
				avalon.vmodels['crud02_datagrid'].hideContent();
				avalon.vmodels['crud02_datagrid'].reloadData(vm.getData());
			}
		},
		$agt_status_opt: {
			data: {'': '全部', '0': '禁用', '1': '可用'}
		...
		$agt_prov_opt: {
			...
			onchanged: function(ev, vm) {
				if(vm.getValue() == '') {
					avalon.vmodels['crud02_city'].removeData();
					avalon.vmodels['crud02_area'].removeData();
				}else {
					avalon.vmodels['crud02_city'].reloadData(vm.getData());
				}
			}
		},
		$agt_city_opt: {
			...
			onchanged: function(ev, vm) {
				if(vm.getValue() == '') {
					...
					avalon.vmodels['crud02_area'].reloadData(vm.getData());
				}
			}
		},
		$agt_area_opt: {
		...
		$datagrid_opt: {
			...
			cols: [
				...
				{name: 'detail', display: '详情', width: 70, type: 'text', disabled: 'true', render: function(val, row, ridx) {
					...
					avalon.vmodels['crud02_dialog'].setTitle(vm.getRow([row])[0].agt_name + '详情');
					avalon.vmodels['crud02_dialog'].showDialog();
				}}
			],
